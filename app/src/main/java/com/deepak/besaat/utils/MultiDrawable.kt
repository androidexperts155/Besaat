 ...
 */
class MultiDrawable(private val mDrawables: List<Drawable>) :
    Drawable() {
    override fun draw(canvas: Canvas) {
        ...
        if (mDrawables.size == 2 || mDrawables.size == 3) { // Paint left half
            canvas.save()
        ...
        if (mDrawables.size >= 4) { // Paint top left
            canvas.save()
            canvas.scale(.5f, .5f)
            mDrawables[0].draw(canvas)
            // Paint bottom left
